...
export function AppShell(): React.JSX.Element {
  ...
  const logoutMutation = useMutation({
    ...
    onSettled: () => {
       ...
       * even if the logout request fails because the
       * API or network is unavailable.
       */
      clearAuthentication();
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        ...
        <div className="mt-8">
          ...
          <div className="space-y-1">
            {workspacesQuery.data?.slice(0, 5).map((workspace) => (
              <NavLink
                key={workspace.id}
      ...
      <div className="border-t border-white/[0.08] p-4">
        <div className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.045] p-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400 to-violet-500 text-xs font-bold text-white">
            {getInitials(user?.firstName, user?.lastName) || "TF"}
          </span>
      ...
      <div className="relative lg:pl-72">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200/75 bg-white/75 px-5 backdrop-blur-xl sm:px-8 lg:px-10">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link
              to="/workspaces/new"
              className="group flex items-center gap-2 rounded-xl bg-slate-950 px-3 py-2.5 text-sm font-semibold text-white shadow-lg shadow-slate-300 transition duration-200 hover:-translate-y-0.5 hover:bg-violet-700 hover:shadow-violet-200 sm:px-4"
            >
              <svg
                aria-hidden="true"
                viewBox="0 0 20 20"
            ...
            <button
              ...
              className="group flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:translate-y-0 disabled:opacity-60 sm:px-4"
            >
              {logoutMutation.isPending ? (
                <span
                  aria-hidden="true"
                  className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 border-t-rose-600"
                />
              ) : (
                <svg
                  aria-hidden="true"
